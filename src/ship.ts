...
function hexPanelTexture() {
  ...
  x.fillStyle = "#c6cbd7"; x.fillRect(0, 0, s, s);
  for (let i = 0; i < 500; i++) { x.fillStyle = `rgba(255,255,255,${Math.random() * 0.05})`; x.fillRect(Math.random() * s, 0, 1, s); }
  x.fillStyle = "#14161d";
  const hex = (cx, cy, r) => { x.beginPath(); for (let i = 0; i < 6; i++) { const a = Math.PI / 3 * i + 0.35; const px = cx + Math.cos(a) * r, py = cy + Math.sin(a) * r; i ? x.lineTo(px, py) : x.moveTo(px, py); } x.closePath(); x.fill(); };
  for (let i = 0; i < 50; i++) hex(20 + Math.random() * (s - 40), 20 + Math.random() * (s - 40), 4 + Math.random() * 9);
...
function panelRoughnessTexture() {
  const s = 256, c = document.createElement("canvas"); c.width = c.height = s;
  const x = c.getContext("2d");
  x.fillStyle = "#7a7a7a"; x.fillRect(0, 0, s, s);           // base roughness ~0.48
  x.strokeStyle = "rgba(255,255,255,0.55)"; x.lineWidth = 1;  // seams read rougher
  for (let i = 0; i < 14; i++) { const y = Math.random() * s; x.beginPath(); x.moveTo(0, y); x.lineTo(s, y + (Math.random() - 0.5) * 30); x.stroke(); }
  for (let i = 0; i < 10; i++) { const gx = Math.random() * s; x.beginPath(); x.moveTo(gx, 0); x.lineTo(gx + (Math.random() - 0.5) * 30, s); x.stroke(); }
  for (let i = 0; i < 2500; i++) { x.fillStyle = `rgba(${Math.random() > 0.5 ? 255 : 0},${0},${0},${Math.random() * 0.06})`; x.fillRect(Math.random() * s, Math.random() * s, 1, 1); }
...
export function createShip() {
  ...
  hull.rotation.x = -Math.PI / 2; hull.position.y = -0.17;
  craft.add(hull);
  ...
  deck.rotation.x = -Math.PI / 2; deck.position.y = 0.19;
  craft.add(deck);
  ...
  for (const sx of [-1, 1]) {
    for (let i = 0; i < 3; i++) {
      ...
      vent.position.set(sx * 0.7, 0.2, -0.9 + i * 0.22); craft.add(vent);
    }
    const pod = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.09, 0.5, 12), trimMat);
    pod.rotation.x = Math.PI / 2; pod.position.set(sx * 1.45, 0.05, -0.2); craft.add(pod);
  }

  // ---- cockpit canopy ----
  const canopy = new THREE.Mesh(new THREE.SphereGeometry(0.3, 24, 16), glassMat);
  canopy.scale.set(1.0, 0.5, 1.9); canopy.position.set(0, 0.26, -1.2);
  craft.add(canopy);

  // ---- engines: nozzles + inner cores + additive plumes ----
  const engines: any[] = [], plumes: any[] = [];
  for (const sx of [-0.55, 0.55]) {
    const nozzle = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.2, 0.35, 18), trimMat);
    nozzle.rotation.x = Math.PI / 2; nozzle.position.set(sx, 0.04, 2.85); craft.add(nozzle);
    const core = new THREE.Mesh(new THREE.SphereGeometry(0.13, 16, 12), engineMat);
    core.position.set(sx, 0.04, 2.95); craft.add(core); engines.push(core);
    const plume = new THREE.Mesh(new THREE.ConeGeometry(0.15, 2.4, 16, 1, true),
      new THREE.MeshBasicMaterial({ color: 0x6fc4ff, transparent: true, opacity: 0.42, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide }));
    plume.rotation.x = -Math.PI / 2; plume.position.set(sx, 0.04, 4.1); craft.add(plume); plumes.push(plume);
  }

  // ---- nav lights + strobe ----
  const navL = new THREE.Mesh(new THREE.SphereGeometry(0.06, 10, 10), new THREE.MeshBasicMaterial({ color: 0xff4d4d }));
  navL.position.set(-1.9, 0.02, -0.4); craft.add(navL);
  const navR = new THREE.Mesh(new THREE.SphereGeometry(0.06, 10, 10), new THREE.MeshBasicMaterial({ color: 0x57ff8c }));
  navR.position.set(1.9, 0.02, -0.4); craft.add(navR);
  const strobe = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), new THREE.MeshBasicMaterial({ color: 0xffffff }));
  strobe.position.set(0, 0.32, -1.7); craft.add(strobe);
  ...
  function updateTrail(p: THREE.Vector3) {
    if (!trailSeeded) { for (let i = 0; i < TRAIL_N; i++) { trailPos[i * 3] = p.x; trailPos[i * 3 + 1] = p.y; trailPos[i * 3 + 2] = p.z; } trailSeeded = true; }
    ...
    for (let i = 0; i < TRAIL_N; i++) { const f = 1 - i / TRAIL_N; trailCol[i * 3] = 0.42 * f; trailCol[i * 3 + 1] = 0.66 * f; trailCol[i * 3 + 2] = 1.0 * f; }
  ...
  function update(time) {
    ...
    navL.scale.setScalar(blink ? 1 : 0.3); navR.scale.setScalar(blink ? 1 : 0.3);
