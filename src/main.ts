...
function extendTrail(idx: number) {
  if (trailIndices[trailIndices.length - 1] === idx) return;
...
function updateFavTrail() {
  const pts = cosmos.anomalies.filter((a: any) => cosmosFavs.has(a.data.name)).map((a: any) => a.group.position);
...
function exitCosmos() {
  ...
  const tourBtn = document.getElementById("cos-tour"); if (tourBtn) tourBtn.textContent = "▶ Auto-tour";
  favTour = false; document.getElementById("cos-fav-tour")?.classList.remove("active");
  const favTourBtn = document.getElementById("cos-fav-tour"); if (favTourBtn) favTourBtn.textContent = "★ Tour my favorites";
  ambientDrift = false; document.getElementById("cos-ambient-drift")?.classList.remove("active");
  setHash("");
}
...
function applyHash() {
  ...
  if (h === "cosmos") enterCosmos();
    ...
    if (!openObjectByName(name)) toast("That object isn't in the catalog.");
  }
...
function refreshStage() {
  ...
  if (idx !== currentStage) {
    ...
    try {
      ...
      if (idx > stored) localStorage.setItem("singularity.furthestStage", String(idx));
...
window.addEventListener("keydown", (e) => {
  ...
  if (page === "cosmos") {                 // WASD / arrows fly the cosmos
    ...
    else if (k >= "1" && k <= "9") showAnomaly(+k - 1);   // jump to entity N
    return;
  }
  if (e.key === "ArrowRight") nudgeProgress(0.04);
    ...
    autoCruise = false; targetProgress = s.t;
    toast(`Stage: ${s.name}`);
  }
    ...
    const el = document.getElementById("c-time") as HTMLInputElement;
...
// ---------- user-saved custom presets (mass/spin/brightness/spectrum) ----------
type CustomPreset = { name: string; mass: number; spin: number; bright: number; pal: number };
...
function renderCustomPresetOptions() {
  const group = document.getElementById("my-presets-group") as HTMLOptGroupElement | null;
  if (!group) return;
...
document.getElementById("my-preset-save-btn")?.addEventListener("click", () => {
  const nameInput = document.getElementById("my-preset-name") as HTMLInputElement;
  const name = (nameInput.value || "").trim().slice(0, 24);
  if (!name) { toast("Give your preset a name first."); return; }
  const pal = parseInt(document.querySelector<HTMLElement>("#c-spectrum .sw.active")?.dataset.pal || "0", 10);
  const list = loadCustomPresets();
...
document.getElementById("my-preset-delete-btn")?.addEventListener("click", () => {
  ...
  if (!val.startsWith("custom:")) { toast("Select one of your saved presets first."); return; }
...
document.getElementById("c-preset")?.addEventListener("change", (e) => {
  ...
  if (val.startsWith("custom:")) {
    ...
    if (!p) return;
    setR("c-mass", p.mass); setR("c-spin", p.spin); setR("c-bright", p.bright);
...
function formatBigNumber(n: number): string {
  ...
  if (Math.abs(n) >= 1e6 || (Math.abs(n) > 0 && Math.abs(n) < 1e-3)) return n.toExponential(3);
...
function updateMassConverter() {
  ...
  if (!isFinite(solar) || solar < 0) { out.innerHTML = ""; return; }
...
document.getElementById("c-quality")?.addEventListener("change", (e) => {
  ...
  if (!q) { perfPinned = false; perfTier = 2; toast("Adaptive quality on"); return; }   // back to auto
...
function updateCartographerProgress() {
  ...
  const seenKinds = new Set(cosmos.anomalies.filter((a: any) => hasViewedObject(a.data.name)).map((a: any) => a.data.kind));
  setCartographerProgress(seenKinds.size, kinds.length);
  if (seenKinds.size >= kinds.length) unlockAchievement("cartographer");
}
...
function openCosmosCard(d) {
  ...
  if (scaleEl) { scaleEl.innerHTML = ly > 0 ? distancePerspective(ly) : ""; scaleEl.style.display = ly > 0 ? "" : "none"; }
...
document.getElementById("cc-link")?.addEventListener("click", () => {
  ...
  navigator.clipboard?.writeText(url).then(
    () => toast("Link to this entity copied to clipboard."),
    () => toast(url),
  );
});
...
document.getElementById("cc-download")?.addEventListener("click", () => {
  ...
  const imgSrc = (document.getElementById("cc-img") as HTMLImageElement).src;
  ...
  const render = () => {
    let y = imgH + 60;
    ctx.fillStyle = "#ffd766"; ctx.font = "13px sans-serif";
    ctx.fillText(kind.toUpperCase(), 40, y - 30);
    ctx.fillStyle = "#ffffff"; ctx.font = "bold 40px sans-serif";
    ctx.fillText(name, 40, y); y += 36;
    ctx.font = "16px monospace"; ctx.fillStyle = "#7d8bb3";
    ctx.fillText(dist, 40, y); y += 46;
    ctx.font = "17px sans-serif"; ctx.fillStyle = "#e8ecf7";
    if (ctx.measureText(blurb).width <= W - 80) {
      ...
      words.forEach((w, i) => {
        ...
        if (ctx.measureText(test).width > W - 80 && line) { ctx.fillText(line, 40, y); y += 24; line = w; }
        else line = test;
...
function stepAnomaly(dir: number) {        // step to the next *visible* (filtered) entity
  const n = cosmos.anomalies.length;
  let i = cardIndex;
  for (let k = 0; k < n; k++) { i = ((i + dir) % n + n) % n; if (cosmos.anomalies[i].group.visible) { showAnomaly(i); return; } }
}
...
document.getElementById("cos-fly-fav")?.addEventListener("click", () => {
  ...
  cosmos.anomalies.forEach((a: any, i: number) => {
    if (!cosmosFavs.has(a.data.name)) return;
...
document.getElementById("cos-fav-trail-toggle")?.addEventListener("click", (e) => {
  ...
  (e.currentTarget as HTMLElement).classList.toggle("active", favTrailLine.visible);
...
document.getElementById("cos-rings-toggle")?.addEventListener("click", (e) => {
  ...
  (e.currentTarget as HTMLElement).classList.toggle("active", depthRingGroup.visible);
...
document.getElementById("cos-ambient-drift")?.addEventListener("click", (e) => {
  ...
  (e.currentTarget as HTMLElement).classList.toggle("active", ambientDrift);
...
function favTourIndices(): number[] {
  return cosmos.anomalies.map((a: any, i: number) => i).filter((i: number) => cosmosFavs.has(cosmos.anomalies[i].data.name));
}
function updateFavTourLabel(idxs: number[]) {
...
function stopFavTour(paused = false) {
  ...
  updateFavTourLabel([]);
  if (paused) toast("Favorites tour paused — you have the controls.");
}
document.getElementById("cos-fav-tour")?.addEventListener("click", () => {
  const idxs = favTourIndices();
  if (!idxs.length) { toast("No cosmos favorites yet — star an entity first."); return; }
  if (tour) { tour = false; document.getElementById("cos-tour")?.classList.remove("active"); updateTourLabel(); }
  favTour = !favTour; tourT = 0; favTourI = 0;
  document.getElementById("cos-fav-tour")?.classList.toggle("active", favTour);
  if (favTour) { openCosmosCard(cosmos.focus(idxs[0])); updateFavTourLabel(idxs); }
  else { cosmosCard.classList.remove("open"); updateFavTourLabel(idxs); }
...
mapCanvas.addEventListener("click", (e) => {
  ...
  let bi = -1, bd = 11;
  cosmos.anomalies.forEach((a, i) => { const [ax, ay] = worldToMap(a.group.position.x, a.group.position.z); const d = Math.hypot(ax - mx, ay - my); if (d < bd) { bd = d; bi = i; } });
...
function bumpCaptureCount() {
  try {
    ...
    if (n >= 5) unlockAchievement("shutterbug");
  } catch (e) { /* storage unavailable */ }
}
...
function renderCollection() {
  ...
  if (cosGrid) {
    const cosFavList = [...cosmosFavs].map(name => {
      const idx = ANOMALIES.findIndex((a: any) => a.name === name);
...
window.addEventListener("keydown", (e) => { if (e.key === "Escape") toggleCollection(false); });
...
document.getElementById("collection-poster-btn")?.addEventListener("click", () => {
  ...
  if (!items.length) { toast("Favorite some objects first — tap ☆ on any card."); return; }
  ...
  ctx.font = "16px sans-serif"; ctx.fillStyle = "#7d8bb3";
  ...
  items.forEach((item, i) => {
    ...
    img.onload = img.onerror = () => {
      ...
      if (img.complete && img.naturalWidth) {
        ...
        const dw = img.width * scale, dh = img.height * scale;
        ctx.save();
        ctx.beginPath(); ctx.rect(x, y, cellW, cellH - 36); ctx.clip();
        ctx.drawImage(img, x + (cellW - dw) / 2, y + (cellH - 36 - dh) / 2, dw, dh);
        ctx.restore();
      }
      ctx.fillStyle = "#e8ecf7"; ctx.font = "15px sans-serif"; ctx.textAlign = "center";
...
function recordPersonalBest(key: string, value: number, opts: { higherIsBetter?: boolean; announce?: string } = {}) {
  try {
...
function getPersonalBest(key: string): number | null {
  try { const v = parseFloat(localStorage.getItem("singularity.pb." + key) || ""); return isNaN(v) ? null : v; } catch (e) { return null; }
}
...
function renderVisitHeatmap() {
  ...
  try { dates = new Set(JSON.parse(localStorage.getItem("singularity.visitDates") || "[]")); } catch (e) { dates = new Set(); }
  const days = 84;
...
function renderNotesSearch() {
  ...
  if (!list) return;
  const q = ((document.getElementById("help-notes-search") as HTMLInputElement)?.value || "").trim().toLowerCase();
  const notes = getAllNotes();
  const entries = Object.entries(notes).filter(([name, text]) =>
    !q || name.toLowerCase().includes(q) || text.toLowerCase().includes(q));
...
document.getElementById("help-notes-list")?.addEventListener("click", (e) => {
  const btn = (e.target as HTMLElement).closest("button[data-note-name]") as HTMLElement | null;
  if (!btn) return;
  toggleHelp(false);
  document.querySelector<HTMLElement>('.nav-pills button[data-view="catalog"]')?.click();
...
document.getElementById("help-recent-fav-list")?.addEventListener("click", (e) => {
  const btn = (e.target as HTMLElement).closest("button[data-recent-fav-name]") as HTMLElement | null;
  if (!btn) return;
  toggleHelp(false);
...
type CmdItem = { label: string; sub: string; action: () => void };
const cmdkItems: CmdItem[] = [
  { label: "Simulator", sub: "view", action: () => document.querySelector<HTMLElement>('.nav-pills button[data-view="sim"]')?.click() },
  { label: "Anatomy", sub: "view", action: () => document.querySelector<HTMLElement>('.nav-pills button[data-view="features"]')?.click() },
  { label: "Catalog", sub: "view", action: () => document.querySelector<HTMLElement>('.nav-pills button[data-view="catalog"]')?.click() },
...
function renderCmdk() {
  ...
  cmdkResults.innerHTML = cmdkMatches.map((it, i) =>
    `<div class="cmdk-item${i === 0 ? " active" : ""}" data-i="${i}"><span class="cmdk-label">${it.label}</span><span class="cmdk-sub">${it.sub}</span></div>`
  ).join("");
}
...
cmdkResults.addEventListener("click", (e) => {
  ...
  if (item) activateCmdk(parseInt(item.dataset.i, 10));
});
cmdkOverlay.addEventListener("click", (e) => { if (e.target === cmdkOverlay) toggleCmdk(false); });
cmdkInput.addEventListener("keydown", (e) => {
  ...
  else if (e.key === "Escape") { e.preventDefault(); toggleCmdk(false); }
});
window.addEventListener("keydown", (e) => {
  if ((e.key === "k" || e.key === "K") && (e.metaKey || e.ctrlKey)) { e.preventDefault(); toggleCmdk(true); }
});
...
document.getElementById("help-export-notes")?.addEventListener("click", () => {
  ...
  if (!names.length) { toast("No notes saved yet — jot one on any object's detail page."); return; }
  const text = names.map(name => `${name}\n${"-".repeat(name.length)}\n${notes[name]}`).join("\n\n");
  const blob = new Blob([text], { type: "text/plain" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = "singularity-notes.txt";
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
...
document.getElementById("help-download-badge")?.addEventListener("click", () => {
  ...
  ctx.font = "16px sans-serif"; ctx.fillStyle = "#ffd98a";
  ...
  list.forEach((a, i) => {
    ...
    ctx.lineWidth = 1.5;
    roundRect(ctx, x, y, cellW - pad, cellH, 14); ctx.fill(); ctx.stroke();
    ctx.font = "30px sans-serif"; ctx.fillStyle = a.unlocked ? "#ffd98a" : "#5a6280";
    ctx.fillText(a.unlocked ? "🏆" : "🔒", x + 16, y + 46);
    ctx.font = "bold 15px sans-serif"; ctx.fillStyle = a.unlocked ? "#ffffff" : "#7d8bb3";
    ctx.fillText(a.label, x + 60, y + 34);
    ctx.font = "12.5px sans-serif"; ctx.fillStyle = "#8b96b8";
...
function wrapCanvasText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number, maxLines = 2) {
  const words = text.split(/\s+/);
  let line = "", lines = 0;
  for (const word of words) {
    const test = line ? line + " " + word : word;
    if (ctx.measureText(test).width > maxWidth && line) {
      ctx.fillText(line, x, y); y += lineHeight; line = word; lines++;
      if (lines >= maxLines - 1) { ctx.fillText(words.slice(words.indexOf(word)).join(" "), x, y); return; }
    } else line = test;
  }
...
const visitStreak = (() => {
  try {
    ...
    const datesSet = new Set<string>(JSON.parse(localStorage.getItem("singularity.visitDates") || "[]"));
    datesSet.add(today);
...
(function heroFactRotator() {
  ...
  if (!el || !textEl) return;
  const facts = ALL_OBJECTS.map((o: any) => o.fact).filter(Boolean);
...
(function catalogFactBanner() {
  ...
  const yearOf = (s: string) => { const m = /\b(1[5-9]\d{2}|20\d{2})\b/.exec(s || ""); return m ? parseInt(m[0], 10) : NaN; };
  const spinOf = (s: string) => { const m = /([\d.]+)/.exec(s || ""); return m ? parseFloat(m[1]) : NaN; };

  const byMass = [...ALL_OBJECTS].filter((o: any) => parseSci(o.mass || "") > 0).sort((a: any, b: any) => parseSci(b.mass) - parseSci(a.mass));
  const byDist = [...ALL_OBJECTS].filter((o: any) => parseSci(o.distance || "") > 0).sort((a: any, b: any) => parseSci(a.distance) - parseSci(b.distance));
  const byYear = [...ALL_OBJECTS].map((o: any) => ({ o, y: yearOf(o.discovered) })).filter(x => !isNaN(x.y)).sort((a, b) => a.y - b.y);
  const bySpin = [...ALL_OBJECTS].map((o: any) => ({ o, s: spinOf(o.spin) })).filter(x => !isNaN(x.s)).sort((a, b) => b.s - a.s);
  const nQuasar = ALL_OBJECTS.filter((o: any) => o.category === "quasar").length;
  const nPulsar = ALL_OBJECTS.filter((o: any) => o.category === "pulsar").length;
  const nBlackhole = ALL_OBJECTS.filter((o: any) => o.category === "blackhole").length;
  const nConstellations = new Set(ALL_OBJECTS.map((o: any) => o.constellation).filter(Boolean)).size;

  const facts: string[] = [];
  if (byMass.length) facts.push(`The heaviest object here is <b>${byMass[0].name}</b>, at ${byMass[0].mass}.`);
  if (byDist.length) facts.push(`The nearest object is <b>${byDist[0].name}</b>, ${byDist[0].distance} away.`);
  if (byDist.length) facts.push(`The most distant object is <b>${byDist[byDist.length - 1].name}</b>, ${byDist[byDist.length - 1].distance} away.`);
  if (byYear.length) facts.push(`The oldest discovery is <b>${byYear[0].o.name}</b>, identified in ${byYear[0].y}.`);
  if (byYear.length) facts.push(`The most recent discovery is <b>${byYear[byYear.length - 1].o.name}</b>, from ${byYear[byYear.length - 1].y}.`);
  if (bySpin.length) facts.push(`The fastest-spinning object is <b>${bySpin[0].o.name}</b>, at a≈${bySpin[0].s.toFixed(2)}.`);
...
function applyPerfTier() {
  if (perfTier >= 2) {
    ...
    bloomPass.strength = parseFloat((document.getElementById("c-bloom") as HTMLInputElement)?.value || "0.7");
  } else if (perfTier === 1) {
...
function tick() {
  ...
  if (page === "cosmos") {
    ...
    if (favTour) {
      ...
      if (tourT > tourInterval) {
        tourT = 0;
        const idxs = favTourIndices();
        if (!idxs.length) { stopFavTour(); }
        else { favTourI = (favTourI + 1) % idxs.length; openCosmosCard(cosmos.focus(idxs[favTourI])); updateFavTourLabel(idxs); }
      }
    }
    if (ambientDrift) cosmos.panBy(Math.sin(time * 0.15) * dt * 0.05, Math.cos(time * 0.11) * dt * 0.05);
  ...
  else audio.setIntensity(cosmos.zoom * 0.85);   // the drone opens up the deeper you dive into the cosmos
  ...
  if (mode === "journey" && progress >= 0.999) {
    ...
    if (journeyStartTime !== null) {
      recordPersonalBest("fastestHorizon", (performance.now() - journeyStartTime) / 1000,
        { announce: "🏅 New personal best — fastest to the horizon!" });
