...
export function createCosmos(renderer) {
  ...
  let streakTint: THREE.Color | null = null;   // set while warping toward an entity
  ...
  return {
    ...
    filterNames(names: Set<string> | null) { for (const a of anomalies) a.group.visible = !names || names.has(a.data.name); },
    ...
    gotoViewpoint(v: { zoom: number; panX: number; panY: number }) {
