...
(function wireHudTooltips() {
    ...
    hudEl.addEventListener("mouseover", (e) => {
        const label = e.target.closest("[data-hud-tip]");
        if (label)
            tooltip.textContent = label.dataset.hudTip || "";
    });
...
const CAT_TAB_KEYS = { "1": "all", "2": "blackhole", "3": "quasar", "4": "pulsar" };
window.addEventListener("keydown", (e) => {
    if (!(e.key in CAT_TAB_KEYS) || e.metaKey || e.ctrlKey
        || !document.getElementById("panel-catalog")?.classList.contains("open")
        || document.querySelector("input:focus, textarea:focus"))
        return;
    document.querySelector(`.cat-tabs button[data-cat="${CAT_TAB_KEYS[e.key]}"]`)?.click();
});
...
function loadMcHistory() {
    try {
        return JSON.parse(localStorage.getItem(MC_HISTORY_KEY) || "[]");
    }
    catch (e) {
        return [];
    }
...
function recordMcHistory() {
    ...
    try {
        localStorage.setItem(MC_HISTORY_KEY, JSON.stringify(hist));
    }
    catch (e) { }
    renderMcHistory();
}
...
document.getElementById("c-high-contrast").checked = highContrast;
...
document.getElementById("cos-next-unviewed")?.addEventListener("click", () => {
    let bestIdx = -1, bestDist = Infinity;
    cosmos.anomalies.forEach((a, i) => {
        if (hasViewedObject(a.data.name))
            return;
        const d = a.group.position.distanceTo(cosmos.camera.position);
        if (d < bestDist) {
            bestDist = d;
...
(function wireCosmosSearch() {
    ...
    recentEl?.addEventListener("mousedown", (e) => {
        ...
        const i = cosmos.anomalies.findIndex((a) => a.data.name === name);
        if (i >= 0) {
            showAnomaly(i);
            saveRecent(name);
            input.value = "";
    ...
    const jump = () => {
        ...
        if (i >= 0) {
            showAnomaly(i);
            saveRecent(cosmos.anomalies[i].data.name);
            input.blur();
        }
        else
...
(function buildCosmosFilter() {
    ...
    bar.addEventListener("mouseover", (e) => {
        ...
        if (explain)
            kindTooltip.textContent = explain;
    });
    bar.addEventListener("mousemove", (e) => {
        const b = e.target.closest(".cfilter");
        if (!kindTooltip)
            return;
        if (!b || !COSMOS_KIND_GLOSSARY[b.dataset.kind || ""]) {
            kindTooltip.classList.remove("show");
            return;
        }
        kindTooltip.style.left = e.clientX + "px";
        kindTooltip.style.top = e.clientY + "px";
...
(function renderCosmosBreakdown() {
    ...
    const entries = kinds.map((k) => {
        const matching = cosmos.anomalies.filter((a) => a.data.kind === k);
        const color = matching.length ? "#" + matching[0].data.color.toString(16).padStart(6, "0") : "#888";
        return { kind: k, count: matching.length, color };
    });
    barEl.innerHTML = entries.map((e) => `<span style="width:${(e.count / total * 100).toFixed(2)}%; background:${e.color}"></span>`).join("");
    legendEl.innerHTML = entries.map((e) => `<div><span class="dot" style="background:${e.color}"></span>${e.kind} <b>${e.count}</b></div>`).join("");
})();
...
document.getElementById("cos-bookmark-save")?.addEventListener("click", () => {
    try {
        localStorage.setItem(COS_BOOKMARK_KEY, JSON.stringify(cosmos.getViewpoint()));
    }
    catch (e) { }
...
document.getElementById("cos-bookmark-goto")?.addEventListener("click", () => {
    let v = null;
    try {
        v = JSON.parse(localStorage.getItem(COS_BOOKMARK_KEY) || "null");
    }
    catch (e) { }
    if (!v) {
        toast("No bookmarked view yet — save one first.");
        return;
    }
    cosmosCard.classList.remove("open");
    cosmos.spotlight(-1);
...
document.getElementById("capture-caption-btn")?.addEventListener("click", () => {
    ...
    if (!caption) {
        toast("Type a caption first.");
        return;
    }
    if (!lastCaptureUrl) {
        toast("Capture a frame first (P).");
        return;
    }
    const img = new Image();
    img.onload = () => {
...
document.getElementById("tool-share")?.addEventListener("click", () => {
    ...
    if (page === "bh") {
        const pal = document.querySelector("#c-spectrum .sw.active")?.dataset.pal || "0";
...
document.getElementById("collection-cosmos-grid")?.addEventListener("click", (e) => {
    const removeBtn = e.target.closest("[data-remove-cos]");
    if (removeBtn) {
        e.stopPropagation();
...
function renderPersonalBests() {
    ...
    if (liveEl) {
        if (longest === null || longest <= 0)
            liveEl.textContent = "";
        else if (currentSessionTime >= longest)
            liveEl.textContent = "🔥 You're setting a new personal best right now!";
        else
            liveEl.textContent = `You're ${formatStatsTime(longest - currentSessionTime)} away from your best session (${formatStatsTime(longest)}).`;
    }
}
...
(function applySharedParams() {
    ...
    if (!sp.has("mass") && !sp.has("spin") && !sp.has("bright") && !sp.has("pal"))
        return;
    const setR = (id, v) => { const el = document.getElementById(id); if (el) {
        el.value = v;
        el.dispatchEvent(new Event("input"));
    } };
    if (sp.has("mass"))
        setR("c-mass", sp.get("mass"));
    if (sp.has("spin"))
        setR("c-spin", sp.get("spin"));
    if (sp.has("bright"))
        setR("c-bright", sp.get("bright"));
    const pal = sp.get("pal");
    if (pal && /^[0-4]$/.test(pal))
        document.querySelector(`#c-spectrum .sw[data-pal="${pal}"]`)?.click();
...
document.getElementById("help-export-achievements")?.addEventListener("click", () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "singularity-achievements.txt";
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
...
document.getElementById("help-share-achievements")?.addEventListener("click", () => {
    ...
    if (!unlocked.length) {
        toast("No achievements unlocked yet — go explore!");
        return;
    }
    const text = `🏆 My SINGULARITY achievements (${unlocked.length} / ${list.length}): ${unlocked.map(a => a.label).join(", ")}\n` +
        `https://viswa-z1.github.io/BlackHole-simulator/`;
    navigator.clipboard?.writeText(text).then(() => toast("Achievements copied to clipboard."), () => toast(text));
});
...
document.getElementById("help-achievements-list")?.addEventListener("click", (e) => {
    const badge = e.target.closest(".ach.unlocked");
    if (!badge)
        return;
    ...
    ctx.fillStyle = "#070914";
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "rgba(255,217,138,0.14)";
...
document.getElementById("ff-download")?.addEventListener("click", () => {
    ...
    ctx.fillStyle = "#070914";
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "rgba(255,157,60,0.1)";
...
(function renderHeroRecap() {
    ...
    if (!el || !textEl || !lastSessionRecap)
        return;
    const bits = [];
    if (lastSessionRecap.time > 0)
        bits.push(`explored for ${formatStatsTime(lastSessionRecap.time)}`);
    if (lastSessionRecap.viewed > 0)
        bits.push(`viewed ${lastSessionRecap.viewed} object${lastSessionRecap.viewed === 1 ? "" : "s"}`);
    if (lastSessionRecap.depth > 0.01)
        bits.push(`dove ${lastSessionRecap.depth.toFixed(2)} Bly`);
    if (!bits.length)
        return;
...
function tick() {
    ...
    if (page === "cosmos") {
        ...
        sessionMaxDepth = Math.max(sessionMaxDepth, cosmos.zoom * 4.2);
    }
    if (frame % 300 === 0)
        saveSessionStats();
    if (statsTime >= 300)
