...
export function createCosmos(renderer) {
    ...
    function starLayer(count, spread, size) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const t = Math.random();
            if (t < 0.5)
                c.setRGB(0.6 + t * 0.7, 0.7 + t * 0.5, 1.0);
            else
                c.setRGB(1.0, 0.95 - (t - 0.5) * 0.5, 0.85 - (t - 0.5) * 0.9);
    ...
    for (const a of anomalies)
        (byKind[_a = a.data.kind] || (byKind[_a] = [])).push(a);
    ...
    function pick(ndcX, ndcY) {
        ...
        if (hits.length)
            return anomalies.find(a => a.glow === hits[0].object);
        return null;
    }
    return {
        ...
        update(dt, time = 0) {
            ...
            dust.rotation.x += dt * 0.012; // churning dust
