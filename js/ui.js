...
function achievementProgress(id) {
    ...
    if (id === "time-traveler")
        return timeTravelerProgress;
    if (id === "deep-diver")
        return deepDiverProgress;
    if (id === "shutterbug") {
        const n = (() => { try {
            return parseInt(localStorage.getItem("singularity.captureCount") || "0", 10);
        }
        catch (e) {
            return 0;
        } })();
...
export function setTimeTravelerProgress(sec) { timeTravelerProgress = `${Math.min(Math.floor(sec), 300)}s / 5m`; }
let deepDiverProgress = "0.00 / 2 Bly";
export function setDeepDiverProgress(bly) { deepDiverProgress = `${Math.min(bly, 2).toFixed(2)} / 2 Bly`; }
...
export function renderAchievements() {
    ...
    if (fill)
        fill.style.width = (unlockedAchievements.size / total * 100).toFixed(1) + "%";
    if (label)
...
function wireAchievementFilterTabs() {
    ...
    bar.addEventListener("click", (e) => {
        const btn = e.target.closest("button[data-ach-filter]");
        if (!btn)
            return;
        achFilter = btn.dataset.achFilter || "all";
...
export function recordObjectView(name) {
    if (!name)
        return;
...
export function getSessionViewedCount() { return viewedNames.size - SESSION_START_VIEWED; }
...
export function updateNotesCount(countElId, len) {
...
function downloadObjectCard() {
    ...
    const render = () => {
        ...
        y += 40;
        if (alias) {
            ctx.fillStyle = "#ffb977";
            ctx.font = "italic 22px sans-serif";
            ctx.fillText(alias, 40, y);
            y += 44;
        }
...
function openDetail(o) {
    ...
    if (scaleEl && ly > 0) {
        scaleEl.innerHTML = distancePerspective(ly);
...
function wireDetail() {
    ...
    document.getElementById("detail-img")?.addEventListener("click", () => {
        ...
        if (!src || !lightbox || !lightboxImg)
            return;
        lightboxImg.src = src;
        lightboxImg.alt = document.getElementById("detail-img").alt;
    ...
    statsEl?.addEventListener("mouseover", (e) => {
        ...
        if (explain)
            statTooltip.textContent = explain;
    });
    statsEl?.addEventListener("mousemove", (e) => {
        ...
        if (!statTooltip)
            return;
        if (!lab) {
            statTooltip.classList.remove("show");
            return;
        }
        statTooltip.style.left = e.clientX + "px";
        statTooltip.style.top = e.clientY + "px";
    ...
    const stopAutoplay = () => {
        ...
        autoplayTimer = null;
        if (autoplayBtn) {
            autoplayBtn.textContent = "▶ Slideshow";
...
function cmpDiffBadge(key, val, otherVal) {
    if (!CMP_DIFFABLE.has(key))
        return "";
...
let currentCompareA = null, currentCompareB = null;
function openCompare(a, b) {
    ...
    <div class="cmp-col">
      ...
      <h3>${o.name}</h3>
      ${CMP_ROWS.filter(([k]) => a[k] || b[k]).map(([k, lab]) => `<div class="cmp-row"><span>${lab}</span><span class="cmp-row-val"><b>${o[k] || "—"}</b>${cmpDiffBadge(k, o[k], other[k])}</span></div>`).join("")}
    </div>`;
...
function wireFeatureFocus() {
    ...
    modal.addEventListener("click", (e) => { if (e.target === modal)
        close(); });
    document.addEventListener("keydown", (e) => { if (e.key === "Escape" && modal.classList.contains("open"))
        close(); });
}
...
// distance in human terms: light-travel time + a relatable age comparison
export function distancePerspective(ly) {
    const fmt = (n) => n.toLocaleString(undefined, { maximumFractionDigits: 0 });
    let analogy;
    if (ly < 100)
        analogy = "light left there within a human lifetime.";
    else if (ly < 5000)
        analogy = "about as long ago as the Great Pyramid of Giza was built (~4,500 years).";
    else if (ly < 300000)
        analogy = "longer than Homo sapiens has existed as a species (~300,000 years).";
    else if (ly < 66000000)
        analogy = "longer ago than the dinosaurs went extinct (~66 million years).";
    else if (ly < 4600000000)
        analogy = "a meaningful fraction of Earth's entire age (~4.6 billion years).";
    else
        analogy = "close to the age of the universe itself (~13.8 billion years).";
...
function buildCatalog() {
    ...
    const render = () => {
        ...
        else if (by === "discovered")
            list = [...list].sort((a, b) => parseYear(a.discovered) - parseYear(b.discovered));
        else if (by === "favfirst")
            list = [...list].sort((a, b) => (favs.has(b.name) ? 1 : 0) - (favs.has(a.name) ? 1 : 0));
    ...
    document.getElementById("cat-reset-filters")?.addEventListener("click", () => {
        ...
        document.querySelectorAll(".cat-tabs button").forEach(b => b.classList.toggle("active", b.dataset.cat === "all"));
        document.querySelectorAll(".cat-dist-tabs button").forEach(b => b.classList.toggle("active", b.dataset.dist === "all"));
        render();
        toast("Filters reset.");
    });
    ...
    document.getElementById("cmp-random")?.addEventListener("click", () => {
        ...
        if (pool.length < 2) {
            toast("Not enough objects to compare.");
            return;
        }
...
function exportFavoritesCSV() {
    ...
    const rows = [CSV_COLUMNS.join(",")].concat(favorites.map(o => CSV_COLUMNS.map(c => csvEscape(o[c])).join(",")));
    const blob = new Blob([rows.join("\n")], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "singularity-favorites.csv";
    document.body.appendChild(a);
    a.click();
    a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
