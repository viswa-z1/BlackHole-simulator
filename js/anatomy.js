...
function loop() {
    ...
    for (const c of canvases) {
        ...
        if (c.canvas.getBoundingClientRect().bottom < -50)
            continue;
...
function draw(ctx, id, w, h, t) {
    ...
    switch (id) {
        ...
        case "isco": {
            ...
            ctx.setLineDash([2, 5]);
            ctx.strokeStyle = "rgba(255,157,60,0.45)";
            ctx.beginPath();
            ctx.arc(cx, cy, isco, 0, 7);
            ctx.stroke();
            ctx.setLineDash([]);
            const p = (t * 0.25) % 1;
        ...
        case "disk": {
            ...
            for (let ring = 0; ring < 26; ring++) {
                ...
                ctx.globalAlpha = 0.5 + 0.5 * tnorm;
                ctx.stroke();
                ctx.globalAlpha = 1;
            }
            ctx.fillStyle = "#000";
            ctx.beginPath();
            ctx.ellipse(cx, cy, R, R * 0.42, 0, 0, 7);
            ctx.fill();
            break;
        }
        case "lensing": {
            ...
            ctx.strokeStyle = "rgba(91,184,255,0.28)";
            ctx.lineWidth = 1;
            for (let gx = 0; gx <= w; gx += 22) {
                ctx.beginPath();
                for (let gy = 0; gy <= h; gy += 6) {
                    const [x, y] = lens(gx, gy);
        ...
        case "ergosphere": {
            const R = h * 0.18;
            ctx.strokeStyle = "rgba(150,110,255,0.5)";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.ellipse(cx, cy, R * 1.7, R * 1.25, 0, 0, 7);
            ctx.stroke();
            for (let i = 0; i < 22; i++) {
        ...
        case "jets": {
            ...
            for (const dir of [-1, 1]) {
                ...
                ctx.beginPath();
                ctx.moveTo(cx, cy);
                ctx.lineTo(cx + Math.sin(t * 2) * 4, cy + dir * len);
                ctx.stroke();
            }
            // thin disk line
            ctx.strokeStyle = "rgba(255,157,60,0.7)";
            ctx.lineWidth = 3;
            ctx.beginPath();
        ...
        case "doppler": {
            ...
            ctx.fillStyle = "#000";
            ctx.beginPath();
            ctx.ellipse(cx, cy, R * 0.5, R * 0.18, 0, 0, 7);
            ctx.fill();
            break;
        }
        case "redshift": {
            ...
            ctx.beginPath();
            for (let x = 0; x <= w; x += 4) {
                const y = h * 0.78 + Math.exp(-Math.pow((x - cx) / 40, 2)) * h * 0.5;
